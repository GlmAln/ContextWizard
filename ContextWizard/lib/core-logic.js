...
export async function classifyComment(context) {
...
export async function improveComment(context) {
...
export async function postImprovedComment(context, improvedComment, originalCommentId) {
    try {
...
function buildSummaryPrompt(prContext, reviews, comments) {
    const prDetails = `
# PULL REQUEST CONTEXT
...
export async function summarizeReview(prContext, reviews, comments) {
...
export async function generateCandidateReviews(prContext, fullDiff) {
...
The resulting JSON array must be parsable without errors.`;
    ...
    try {
        ...
        if (!jsonString.endsWith(']'))
            jsonString = jsonString + ']';
        const candidates = JSON.parse(jsonString);
